...
def ambient_adjustement():
    with sr.Microphone() as source:
        ...
        #print(actual_path)
        pre_build = open(os.getcwd() + '\libs\pre_build_sentences.txt')
...
def _orders_():
    ...
    try:
        ...
        if 'parió' in order:
            ...
            engine.runAndWait()
            help_ = open(os.getcwd() + '\libs\help.txt')
            ...
            if len(order) == 2:
                ...
                engine.say('buscando')
                engine.runAndWait()
                search = Argsearch3(order[1], order[2], order[3], order[4], order[5], order[6])
                search.search()
            elif len(order) == 8:
                engine.say('buscando')
                engine.runAndWait()
                search = Argsearch3(order[1], order[2], order[3], order[4], order[5], order[6], order[7])
                search.search()
        elif "chiste" or "broma" in order:
            jk = open(os.getcwd() + '\libs\jks.txt')
